...
class DatabaseOps {
    ...
    constructQuery(operationType, fields = {}, finder = null, current_page = 1, current_search = '') {
        let sql = '';
        ...
        switch (operationType) {
            ...
            case `SELECTALL`:
                ...
                const search = current_search ? `WHERE title LIKE '${current_search}'` : ''
            ...
            case `SELECTCOUNT`:
                ...
                sql = current_search ? `SELECT COUNT(*) as total FROM ${this.tableName} ${likeClause}` 
...
class SearchAllData extends DatabaseOps {
    async execute(current_page, current_search = '') {
        const { sql } = this.constructQuery('SELECTALL', {}, null, current_page, current_search);
...
class CountAllData extends DatabaseOps {
    async execute(current_search) {
        const { sql } = this.constructQuery("SELECTCOUNT", {}, null, 1, current_search)
...
class UpdateBoardData extends DatabaseOps {
    async execute(fields, finder) {
        const { sql, valueArr } = this.constructQuery('UPDATEBOARD', fields, finder);
        const [{ changedRows }] = await this.executeQuery(sql, valueArr);
        return !!changedRows;
    }
}
