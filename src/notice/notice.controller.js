...
exports.getWrite = (req, res) => {
    if (req.user.lvl !== 2) res.status(401).send(`관리자만 작성할 수 있습니다.`)
...
exports.getView = async (req, res) => {
    ...
    if (!result) res.status(401).send(`Cannot find this notice.`)
...
exports.postDelete = async (req, res) => {
    if (req.user.lvl !== 2) res.status(401).send(`관리자만 삭제할 수 있습니다.`)
