...
exports.getList = async (req, res) => {
    ...
    const { totalPages, startPage, endPage, result } =
        await noticeService.fetchAllNotices(currentPage, search);
...
exports.getWrite = (req, res) => {
    ...
    if (req.user.lvl !== 2)
        return res.status(401).send(`관리자만 작성할 수 있습니다.`);
...
exports.getModify = async (req, res) => {
    if (!req.user) return res.redirect(`/accounts/signin`);
    if (req.user.lvl !== 2) return res.send(`관리자만 수정할 수 있습니다.`);
...
exports.postDelete = async (req, res) => {
    if (!req.user) return res.redirect(`/accounts/signin`);
    if (req.user.lvl !== 2) return res.send(`관리자만 삭제할 수 있습니다.`);

    const result = await noticeService.deleteNotice(req.query);
    if (!result) return res.send(`Cannot delete.`);
    return res.redirect(`/notice/list`);
};
